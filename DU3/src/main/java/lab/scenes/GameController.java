...
public class GameController {
    ...
    private void startNewGame() {
        log.info("Spouštění hry");
        score = 0;
        speed = 80;
        scoreLabel.setText("Score: 0");

        if (drawingThread != null) {
            log.debug("Zastavuji existující DrawingThread");
            drawingThread.stop();
        }
    ...
    void initialize() {
        ...
        try {
            ...
            log.debug("Načteno {} skóre z repozitáře", scoreDataList.size());
        } catch (Exception e) {
            log.error("Nepodařilo se načíst skóre, používám prázdný seznam", e);
            Alert a = new  Alert(Alert.AlertType.ERROR);
            a.setTitle("Error");
            a.getDialogPane().setContentText("Failed to save score!");
            a.show();
            scoreDataList = List.of();
        }
    ...
    void onSubmit(ActionEvent event) {
        log.info("Odeslání skóre: {} = {}", nameInputBox.getText(), score);
        ...
        try {
            ...
            log.debug("Skóre uloženo do repozitáře");
        } catch (Exception e) {
            log.error("Nepodařilo se uložit skóre", e);
            Alert a = new  Alert(Alert.AlertType.ERROR);
            a.setTitle("Error");
            a.getDialogPane().setContentText("Failed to save score!");
            a.show();
